...
import static com.setlone.token.tools.TokenDefinition.UNCHANGED_SCRIPT;
...
import com.setlone.app.ui.widget.adapter.TabPagerAdapter;
import com.setlone.app.ui.widget.adapter.TokensAdapter;
import com.setlone.app.util.TabUtils;
import com.setlone.app.viewmodel.Erc20DetailViewModel;
import com.setlone.app.widget.ActivityHistoryList;
...
/**
 * TRON 네트워크 전용 토큰 상세 화면
 * Erc20DetailActivity와 유사하지만 TRON 네트워크에 특화된 기능 제공
 */
@AndroidEntryPoint
public class TronDetailActivity extends BaseActivity implements StandardFunctionInterface, BuyCryptoInterface
{
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState)
    {
        ...
        if (savedInstanceState != null && savedInstanceState.containsKey(C.EXTRA_SYMBOL))
        {
            ...
            long chainId = savedInstanceState.getLong(C.EXTRA_CHAIN_ID, TRON_ID);
    ...
    private void initViews()
    {
        ...
        viewPager.setOffscreenPageLimit(detailPages.length);
    ...
    private void getIntentData()
    {
        ...
        symbol = symbol == null ? "TRX" : symbol; // TRON 기본 심볼
    ...
    {
        // TRON 전용 Send 화면으로 이동
        Intent intent = new Intent(this, TronSendActivity.class);
        intent.putExtra(C.Key.WALLET, wallet);
        intent.putExtra(C.EXTRA_CHAIN_ID, token.tokenInfo.chainId);
        intent.putExtra(C.EXTRA_ADDRESS, token.getAddress());
        intent.setFlags(Intent.FLAG_ACTIVITY_MULTIPLE_TASK);
        startActivityForResult(intent, C.TOKEN_SEND_ACTIVITY);
    }
    ...
    {
        // TRON 전용 주소 화면으로 이동
        Intent intent = new Intent(this, TronAddressActivity.class);
        intent.putExtra(C.Key.WALLET, wallet);
        intent.putExtra(C.EXTRA_CHAIN_ID, TRON_ID);
        intent.setFlags(Intent.FLAG_ACTIVITY_MULTIPLE_TASK);
        startActivity(intent);
    }
    ...
    {
        // TRON 네트워크에서는 일부 기능이 제한될 수 있음
        if (actionId == R.string.swap)
        {
